...
def upload_file():
    if request.method == 'POST':
        ...
        with open("db/content-log.log", 'w') as f:
            f.write(prev_data+file_name+"???:???"+str(datetime_ist.strftime('%d:%m:%Y %H:%M:%S'))+"???:???"+current_user.get_id()+"???:???"+request.form["caption-text"]+"\n")
